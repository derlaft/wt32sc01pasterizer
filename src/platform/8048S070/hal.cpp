...
void hw_setup() {
  ...
  touch = new TAMC_GT911(TOUCH_GT911_SDA, TOUCH_GT911_SCL, TOUCH_GT911_INT,
                         TOUCH_GT911_RST, TFT_WIDTH, TFT_HEIGHT);
...
void hw_enable_backlight() {
  // Включить подсветку экрана (в последнюю очередь, чтобы не было видно никаких
  // морганий при запуске)
  pinMode(TFT_BL, OUTPUT);
  digitalWrite(TFT_BL, 1);
}

IRAM_ATTR void update_display(lv_disp_drv_t *disp, const lv_area_t *area,
                              lv_color_t *color_p) {
  if (!is_display_open) {
  ...
  screen->draw16bitRGBBitmap(area->x1, area->y1, area->x2 + 1, area->y2 + 1,
